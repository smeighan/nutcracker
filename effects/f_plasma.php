...
$maxcolors=360;
$roughness=$maxcolors*0.20;
$plasma[0][0]=mt_rand(0,$maxcolors);
$plasma[0][$size]=mt_rand(0,$maxcolors);
$plasma[$size][0]=mt_rand(0,$maxcolors);
$plasma[$size][$size]=mt_rand(0,$maxcolors);
/*$plasma[0][0]=180;
$plasma[0][$size]=200;
$plasma[$size][0]=180;
$plasma[$size][$size]=220;*/
$log_size=log($size);
$log2 = log(2);
echo "<pre>";
echo "size=$size\n";
...
for($f=1;$f<=$maxFrame;$f++)
{
	...
	$color1=$color3="#00FF00";
	$color2=$color4="#0000FF";
	$plasma[0][0]=18 + $roughness * (mt_rand(0,1000)/1000 - .5);
	$plasma[0][$size]=20+ $roughness * (mt_rand(0,1000)/1000 - .5);
	$plasma[$size][0]=18 + $roughness * (mt_rand(0,1000)/1000 - .5);
	$plasma[$size][$size]=22 + $roughness * (mt_rand(0,1000)/1000 - .5);
	for($p = $result;$p>=0;$p--)
	{
		$pow2=	pow(2, $p);
		for($x=0;$x<=$size;$x+=$pow2)
		{
			for($y=0;$y<=$size;$y+=$pow2)
			{
				...
				{
					if(isset($plasma[$x][ $y + $pow2]) and isset($plasma[$x][ $y - $pow2]))
					{
						$average = ($plasma[$x][ $y + $pow2] + $plasma[$x][ $y - $pow2]) / 2;
						$color = $average + $roughness * (mt_rand(0,1000)/1000 - .5);
						//$color=.2;
				...
				{
					if(isset($plasma[$x + $pow2][ $y]) and isset($plasma[$x - $pow2][ $y]))
					{
				...
				{
					if(isset($plasma[$x +$pow2][ $y +$pow2]) 
					and isset($plasma[$x +$pow2][ $x -$pow2])
						and isset($plasma[$x -$pow2][ $x +$pow2]) 
					and isset($plasma[$x -$pow2][ $y -$pow2])
						)
					{

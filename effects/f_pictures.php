		...
		*/
		...
		if($is_animated_gif)
		{
			foreach($file_array as $i=>$file)
			{
				...
				process_frame($file,$image_type,$frame,$get,$offset_left_array[$i],$offset_top_array[$i]);
		...
		else
		{
			...
			for($frame=1;$frame<=$maxFrame;$frame++)
			{
				...
				$dat_file[$frame] = $path . "/" . $x_dat;
				$dat_file_array[] = $dat_file[$frame];
				process_frame($file,$image_type,$frame,$get,0,0);
			}
		}
		echo "<pre>";
...
function process_frame($file,$image_type,$frame,$get,$offset_left,$offset_top)
{
	echo "<pre>function process_frame($file,$image_type,$frame,$get,$offset_left,$offset_top)</pre>\n";
	$image_path=$file;
	extract($get);
/*	echo "<pre>process_frame:\n";
	...
	echo "<pre>frame=$frame, file=$file, fh=" . $dat_file[$frame] . "</pre>\n";
